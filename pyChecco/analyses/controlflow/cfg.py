...
class CFG(pg.ProgramGraph[pg.ProgramGraphNode]):
    ...
    def from_bytecode(bytecode: Bytecode) -> CFG:
        ...
        # MR: That cost me a lot of time. I want all code in the cfg, since this is what the disassembler does.
    ...
    @staticmethod
    def _create_nodes(blocks: ControlFlowGraph) -> Tuple[Dict[int, List[int]], Dict[int, pg.ProgramGraphNode]]:
    ...
    @staticmethod
    def _create_graph(cfg: CFG, edges: Dict[int, List[int]], nodes: Dict[int, pg.ProgramGraphNode]):
        # add nodes to graph
